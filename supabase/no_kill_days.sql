...
begin
  ...
  select coalesce(r.mafia_can_kill, true) into can_kill
  from public.rooms r
  where r.id = me.room_id;

  if coalesce(can_kill, true) then
    update public.rooms
    set
      status = 'vote_reveal',
      announcement = victim_name || ' was voted out.',
    ...
    set
      ...
      next_status = 'dawn',
      next_winner = null,
      next_announcement = victim_name || ' was voted out. Town talks again.'
    where id = me.room_id;
  end if;
end;
$fn_vote$;
